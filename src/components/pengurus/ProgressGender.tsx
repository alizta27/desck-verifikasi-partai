      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <Progress
            ...
            className={`h-2 ${
              isValid ? "[&>div]:bg-green-500" : "[&>div]:bg-red-500"
            }`}
          />
          <div className="flex justify-between text-xs text-muted-foreground">
            <span className="md:w-auto w-[40%]">
              {perempuanCount} dari {totalCount} pengurus (Tidak termasuk
              Biro-Biro)
            </span>
            <span className="md:w-auto w-[40%]">
              {isValid ? "Memenuhi syarat" : "Minimal 30%"}
            </span>
          </div>
          {!isValid && totalCount > 0 && (
            <p className="text-xs text-destructive pt-2">
              Tambahkan minimal {Math.ceil(totalCount * 0.3) - perempuanCount}{" "}
              pengurus perempuan lagi untuk memenuhi syarat 30%
            </p>
          )}
        </div>
      </CardContent>
